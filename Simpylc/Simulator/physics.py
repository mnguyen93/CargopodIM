...
class Physics(sp.Module):
    ...
    def sweep(self):
        ...
        #self.velocity.set(self.velocity + self.acceleration * sp.world.period, self.velocity < self.targetVelocity,
        #                  self.velocity - self.acceleration * sp.world.period)

        error = self.targetVelocity - self.velocity
        self.acceleration.set((self.targetVelocity - self.velocity) / sp.world.period)

        self.velocity.set((error * 0.85) + self.acceleration * sp.world.period,
                          self.velocity < self.targetVelocity,
                          -(error * 0.85) + self.acceleration * sp.world.period)

        midWheelAngleError = self.midSteeringAngle - self.midWheelAngle
        midWheelAngularVelocityError = self.midWheelAngularVelocity - self.midAngularVelocity
        self.midWheelAngularVelocity.set(self.velocity / pm.displacementPerWheelAngle)
        self.midWheelAngle.set((midWheelAngleError * 0.8) + (midWheelAngularVelocityError * 0.85) * sp.world.period)
        self.tangentialVelocity.set((midWheelAngularVelocityError * 0.85) * pm.displacementPerWheelAngle)
        ...
        self.radialAcceleration.set(
            sp.max(abs(self.tangentialVelocity * self.tangentialVelocity * self.inverseMidCurveRadius) - 0.5, 0))
        ...
        self.velocityX.set(
            self.tangentialVelocity * sp.cos(self.courseAngle) + self.radialVelocity * sp.sin(self.courseAngle))
        self.velocityY.set(
            self.tangentialVelocity * sp.sin(self.courseAngle) + self.radialVelocity * sp.cos(self.courseAngle))
